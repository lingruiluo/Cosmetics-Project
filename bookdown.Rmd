...
knitr::kable(df,
             caption = 'Top 10 Ingredients in Products with Low, Medium, High Price')
```
...
skin_df <- com_df %>% rename(Combination = ingredients) %>% select(-times) %>% 
  ...
  inner_join(sen_df %>% rename(Sensitive = ingredients) %>% select(-times)) %>%
                 select(Top, everything())
knitr::kable(skin_df,"html",
             caption = 'Top 10 Ingredients in Products for Each Skin Types') %>%
...
![Skin Type Unique Ingredient Venn Diagram](skin_venn_diagram.png)

```{r echo=FALSE,warning=FALSE}
knitr::kable(
  list(
  data.frame(com_unique_ingredients) %>% rename("Unique Ingredients for Combination"=com_unique_ingredients),
  data.frame(dry_unique_ingredients) %>% rename("Unique Ingredients for Dry"=dry_unique_ingredients),
  data.frame(normal_unique_ingredients) %>% rename("Unique Ingredients for Normal"=normal_unique_ingredients),
  data.frame(oily_unique_ingredients) %>% rename("Unique Ingredients for Oily"=oily_unique_ingredients),
  data.frame(sensitive_unique_ingredients) %>% rename("Unique Ingredients for Sensitive"=sensitive_unique_ingredients)),
  caption = 'The Unique Ingredients in Products for Each Skin Type'
  ) %>%
  kableExtra::kable_styling(bootstrap_options = "striped", full_width = F)
```
